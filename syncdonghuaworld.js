...
/* Descubrir series desde las páginas que confirmaste */
async function discoverSeries() {
  const found = new Set();

  console.log('🔍 Descubriendo series...');

  // Sondeo páginas 1-31 (estructura: / y /page/N/)
  for (let page = 1; page <= 31; page++) {
    ...
    try {
      ...
      // Buscar TODOS los enlaces /anime/ que aparecen en el HTML
      // Formato: href="https://donghuaworld.com/anime/slug/" o href="/anime/slug/"
      const patterns = [
        /href="https?:\/\/donghuaworld\.com\/anime\/([a-z0-9-]+)\/?"/gi,
        /href="\/anime\/([a-z0-9-]+)\/?"/gi
      ];
      ...
      for (const regex of patterns) {
        ...
        while ((match = regex.exec(html)) !== null) {
          const slug = match[1];
          // Filtrar páginas de categoría/tag
          if (slug && slug !== 'page' && !slug.includes('category') && !slug.includes('tag') && !slug.includes('genre')) {
      ...
      // No parar aunque dé 0, seguir hasta página 31 por si acaso
      if (page > 5 && count === 0) {
        console.log(`⚠️ Página ${page} sin series, pero continuo...`);
      }
  ...
  console.log(`\n🎯 Total series encontradas: ${found.size}`);
  return [...found];
}
