...
/* Une las partes de un catálogo dividido (manifiesto sharded) */
function mergeCatalogParts(manifest, parts) {
  const db = { meta: manifest.meta || {}, series: [], seasons: [], episodes: [], genres: [] };
  const gset = new Set(manifest.genres || []);
  for (const p of parts) {
    if (!p) continue;
    if (Array.isArray(p.series)) db.series.push(...p.series);
    if (Array.isArray(p.seasons)) db.seasons.push(...p.seasons);
    if (Array.isArray(p.episodes)) db.episodes.push(...p.episodes);
    if (Array.isArray(p.genres)) p.genres.forEach(g => gset.add(g));
  }
  db.genres = [...gset];
  return db;
}
...
async function fetchCatalogFile(file) {
  const r = await fetch(file + '?ts=' + Date.now(), { cache: 'reload' });
...
async function ensureCatalog(id) {
  ...
  if (data && data.sharded && Array.isArray(data.parts) && data.parts.length) {
    const parts = await Promise.all(data.parts.map(f => fetchCatalogFile(f).catch(() => null)));
...
async function probeCatalogs() {
  for (const c of CATALOGS) {
    ...
    try {
      ...
      if (!r.ok) { CATALOG_AVAILABLE[c.id] = false; continue; }
      const parsed = await r.json().catch(() => null);
      if (parsed && parsed.sharded) {
        CATALOG_AVAILABLE[c.id] = Array.isArray(parsed.parts) && parsed.parts.length > 0;
      } else {
        CATALOG_AVAILABLE[c.id] = Boolean(parsed && Array.isArray(parsed.series));
      }
