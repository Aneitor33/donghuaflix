let DB={series:[],seasons:[],episodes:[],movies:[],genres:[],meta:{}};const app=document.getElementById('app');
...
function search(q=''){const list=DB.series.filter(s=>[s.title,s.originalTitle,s.synopsis,...(s.genres||[])].join(' ').toLowerCase().includes(q.toLowerCase()));app.innerHTML=`<section class="search"><div class="eyebrow">CATÁLOGO</div><h1>Buscar</h1><div class="searchbar"><input id="q" value="${esc(q)}" autofocus placeholder="Serie, título original, género..."></div><div style="margin-top:30px" class="grid">${list.map(card).join('')}</div>${!list.length?'<div class="empty">No encontramos resultados.</div>':''}</section>`;const input=document.getElementById('q');input.oninput=e=>search(e.target.value)}
function movies(){const ms=DB.movies.slice().sort(sortTitle);app.innerHTML=`<section class="section"><div class="section-head"><h2>Películas</h2><span class="muted">${ms.length}</span></div>${ms.length?`<div class="grid">${ms.map(m=>`<article class="card"><div class="poster">${m.image?`<img loading="lazy" src="${esc(m.image)}" alt="${esc(m.title)}">`:''}</div><h3>${esc(m.title)}</h3></article>`).join('')}</div>`:'<div class="empty">No hay películas sincronizadas todavía.</div>'}</section>`}
